...
with open('product-categories.csv',"r") as f:              #读出商品的item_ID对应的种类category_Id
    reader = csv.DictReader(f,delimiter=';')
    item_category = {}
    for data in reader:
        item_id = data['itemId']
...
def obtian_tra():
    ...
    for s, date in tra_sess:
        ...
        for i in seq:
            if i in item_dict:
                ...
                new_item_category[item_ctr] = item_category[i]     #~~~~~~构造新的商品--类别ID的字典索引
    ...
    return train_ids, train_dates, train_seqs,new_item_category
...
tra_ids, tra_dates, tra_seqs,item_category = obtian_tra()       #商品重新标号后的类别字典item_category
...
if opt.dataset == 'diginetica':
    ...
    pickle.dump(item_category,open('diginetica/category.txt', 'wb'))
